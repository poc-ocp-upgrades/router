...
func TestFirstMatch(t *testing.T) {
	...
	testCases := []struct {
		...
		inputs	[]string
		match	string
	}{{name: "exact match no-substring", pattern: `asd`, inputs: []string{"123asd123", "asd456", "123asd", "asd"}, match: "asd"}, {name: "don't match newline", pattern: `.*asd.*`, inputs: []string{"123\nasd123", "123asd123", "asd"}, match: "123asd123"}, {name: "match newline", pattern: `(?s).*asd.*`, inputs: []string{"123\nasd123", "123asd123"}, match: "123\nasd123"}, {name: "match multiline", pattern: `(?m)(^asd\d$\n?)+`, inputs: []string{"asd1\nasd2\nasd3\n", "asd1"}, match: "asd1\nasd2\nasd3\n"}, {name: "don't match multiline", pattern: `(^asd\d$\n?)+`, inputs: []string{"asd1\nasd2\nasd3\n", "asd1", "asd2"}, match: "asd1"}, {name: "prefix alternation", pattern: `|asd`, inputs: []string{"anything"}, match: ""}, {name: "postfix alternation", pattern: `asd|`, inputs: []string{"anything"}, match: ""}, {name: "substring behavior", pattern: `(?m)asd`, inputs: []string{"asd\n123"}, match: ""}}
...
func TestGenerateRouteRegexp(t *testing.T) {
	...
	tests := []struct {
		...
		match		[]string
		nomatch		[]string
	}{{name: "no path", hostname: "example.com", path: "", wildcard: false, match: []string{"example.com", "example.com:80", "example.com/", "example.com/sub", "example.com/sub/"}, nomatch: []string{"other.com"}}, {name: "root path with trailing slash", hostname: "example.com", path: "/", wildcard: false, match: []string{"example.com", "example.com:80", "example.com/", "example.com/sub", "example.com/sub/"}, nomatch: []string{"other.com"}}, {name: "subpath with trailing slash", hostname: "example.com", path: "/sub/", wildcard: false, match: []string{"example.com/sub/", "example.com/sub/subsub"}, nomatch: []string{"other.com", "example.com", "example.com:80", "example.com/", "example.com/sub", "example.com/subpar"}}, {name: "subpath without trailing slash", hostname: "example.com", path: "/sub", wildcard: false, match: []string{"example.com/sub", "example.com/sub/", "example.com/sub/subsub"}, nomatch: []string{"other.com", "example.com", "example.com:80", "example.com/", "example.com/subpar"}}, {name: "wildcard", hostname: "www.example.com", path: "/", wildcard: true, match: []string{"www.example.com", "www.example.com/", "www.example.com/sub", "www.example.com/sub/", "www.example.com:80", "www.example.com:80/", "www.example.com:80/sub", "www.example.com:80/sub/", "foo.example.com", "foo.example.com/", "foo.example.com/sub", "foo.example.com/sub/"}, nomatch: []string{"wwwexample.com", "foo.bar.example.com"}}, {name: "non-wildcard", hostname: "www.example.com", path: "/", wildcard: false, match: []string{"www.example.com", "www.example.com/", "www.example.com/sub", "www.example.com/sub/", "www.example.com:80", "www.example.com:80/", "www.example.com:80/sub", "www.example.com:80/sub/"}, nomatch: []string{"foo.example.com", "foo.example.com/", "foo.example.com/sub", "foo.example.com/sub/", "wwwexample.com", "foo.bar.example.com"}}}
...
func TestMatchPattern(t *testing.T) {
	...
	testMatches := []struct {
		...
		pattern	string
		input	string
	}{{name: "exact match", pattern: `asd`, input: "asd"}, {name: "basic regex", pattern: `.*asd.*`, input: "123asd123"}, {name: "match newline", pattern: `(?s).*asd.*`, input: "123\nasd123"}, {name: "match multiline", pattern: `(?m)(^asd\d$\n?)+`, input: "asd1\nasd2\nasd3\n"}}
	testNoMatches := []struct {
		name	string
		pattern	string
		input	string
	}{{name: "no-substring", pattern: `asd`, input: "123asd123"}, {name: "prefix alternation", pattern: `|asd`, input: "anything"}, {name: "postfix alternation", pattern: `asd|`, input: "anything"}, {name: "substring behavior", pattern: `(?m)asd`, input: "asd\n123"}, {name: "don't match newline", pattern: `.*asd.*`, input: "123\nasd123"}, {name: "don't match multiline", pattern: `(^asd\d$\n?)+`, input: "asd1\nasd2\nasd3\n"}}
...
func TestGenerateHAProxyMap(t *testing.T) {
	...
	defer _logClusterCodePath()
	td := templateData{WorkingDir: "/path/to", State: buildTestTemplateState(), ServiceUnits: make(map[string]ServiceUnit)}
	wildcardDomainOrder := []string{`^[^\.]*\.foo\.wildcard\.test(:[0-9]+)?(/.*)?$`, `^[^\.]*\.foo\.127\.0\.0\.1\.nip\.io(:[0-9]+)?(/.*)?$`, `^[^\.]*\.127\.0\.0\.1\.nip\.io(:[0-9]+)?(/.*)?$`}
	...
	httpBackendOrder := []string{"be_edge_http:zzz:zed-route", "be_edge_http:dev:api-route", "be_edge_http:devel2:foo-wildcard-test", "be_edge_http:devel2:foo-wildcard-route"}
	...
	edgeReencryptOrder := []string{"be_edge_http:test:api-route", "be_edge_http:zzz:zed-route", "be_secure:dev:reencrypt-route", "be_edge_http:prod:backend-route", "be_edge_http:stg:api-route", "be_edge_http:prod:api-path-route", "be_edge_http:prod:api-route", "be_edge_http:dev:api-route", "be_edge_http:dev:admin-route", "be_edge_http:devel2:foo-wildcard-test", "be_edge_http:devel2:foo-wildcard-route", "be_edge_http:prod:wildcard-route"}
	...
	httpRedirectOrder := []string{"test:api-route", "dev:reencrypt-route", "prod:backend-route", "stg:api-route", "prod:api-route"}
	...
	passthroughOrder := []string{"dev:reencrypt-route", "prod:pt-route", "dev:pt-route"}
	lines = generateHAProxyMap("os_tcp_be.map", td)
	if err := checkExpectedOrderSuffixes(lines, passthroughOrder); err != nil {
		t.Errorf("TestGenerateHAProxyMap os_tcp_be.map error: %v", err)
	}
	sniPassthroughOrder := []string{`^passthrough-prod\.127\.0\.0\.1\.nip\.io(:[0-9]+)?(/.*)?$`, `^passthrough-dev\.127\.0\.0\.1\.nip\.io(:[0-9]+)?(/.*)?$`}
	...
	certBackendOrder := []string{"/path/to/certs/zzz:zed-route.pem", "/path/to/certs/test:api-route.pem", "/path/to/certs/stg:api-route.pem", "/path/to/certs/prod:wildcard-route.pem", "/path/to/certs/prod:backend-route.pem", "/path/to/certs/prod:api-route.pem", "/path/to/certs/prod:api-path-route.pem", "/path/to/certs/devel2:foo-wildcard-test.pem", "/path/to/certs/devel2:foo-wildcard-route.pem", "/path/to/certs/dev:reencrypt-route.pem", "/path/to/certs/dev:api-route.pem", "/path/to/certs/dev:admin-route.pem"}
...
func TestGetPrimaryAliasKey(t *testing.T) {
	...
	testCases := []struct {
		...
		expected	string
	}{{name: "zero input", input: make(map[string]ServiceAliasConfig), expected: ""}, {name: "Single alias", input: map[string]ServiceAliasConfig{"project2:route1": {Host: "example.org", Path: "/v1"}}, expected: "project2:route1"}, {name: "Aliases with Edge Termination", input: map[string]ServiceAliasConfig{"project1:route-3": {Host: "example.com", Path: "/", TLSTermination: routev1.TLSTerminationEdge}, "project1:route-1": {Host: "example.com", Path: "/path1", TLSTermination: routev1.TLSTerminationEdge}, "project1:route-2": {Host: "example.com", Path: "/path2", TLSTermination: routev1.TLSTerminationEdge}, "project1:route-4": {Host: "example.com", Path: "/path4"}}, expected: "project1:route-3"}, {name: "Aliases with Reencrypt Termination", input: map[string]ServiceAliasConfig{"project1:route-3": {Host: "example.com", Path: "/", TLSTermination: routev1.TLSTerminationReencrypt}, "project1:route-1": {Host: "example.com", Path: "/path1", TLSTermination: routev1.TLSTerminationReencrypt}, "project1:route-2": {Host: "example.com", Path: "/path2", TLSTermination: routev1.TLSTerminationReencrypt}, "project1:route-4": {Host: "example.com", Path: "/path4"}}, expected: "project1:route-3"}, {name: "Non-TLS aliases", input: map[string]ServiceAliasConfig{"project1:route-3": {Host: "example.com", Path: "/"}, "project1:route-1": {Host: "example.com", Path: "/path1"}, "project1:route-2": {Host: "example.com", Path: "/path2"}, "project1:route-4": {Host: "example.com", Path: "/path4"}}, expected: "project1:route-4"}}
