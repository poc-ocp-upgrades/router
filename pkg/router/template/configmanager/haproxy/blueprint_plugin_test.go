...
type fakeConfigManager struct{ blueprints map[string]*routev1.Route }
...
func TestHandleRoute(t *testing.T) {
	...
	route := &routev1.Route{ObjectMeta: metav1.ObjectMeta{CreationTimestamp: original, Namespace: "bp", Name: "chevron"}, Spec: routev1.RouteSpec{Host: "www.blueprints.org", To: routev1.RouteTargetReference{Name: "TestService", Weight: new(int32)}}}
...
func TestHandleEndpoints(t *testing.T) {
	...
	v2Endpoints := &kapi.Endpoints{ObjectMeta: metav1.ObjectMeta{Namespace: "bpe", Name: "shell"}, Subsets: []kapi.EndpointSubset{{Addresses: []kapi.EndpointAddress{{IP: "1.1.1.1"}, {IP: "2.2.2.2"}}, Ports: []kapi.EndpointPort{{Port: 9876}, {Port: 8888}}}}}
	cm := newFakeConfigManager()
	plugin := NewBlueprintPlugin(cm)
