...
func TestNewClient(t *testing.T) {
	...
	testCases := []struct {
		name		string
		sockFile	string
	}{{name: "empty sockfile", sockFile: ""}, {name: "some sockfile", sockFile: "/tmp/some-fake-haproxy.sock"}, {name: "bad socketfile", sockFile: "/non-existent/fake-haproxy.sock"}}
...
func TestClientRunCommand(t *testing.T) {
	...
	}{{name: "empty sockfile", sockFile: "", failureExpected: true}, {name: "bad socketfile", sockFile: "/non-existent/fake-haproxy.sock", failureExpected: true}, {name: "valid sockfile", sockFile: server.SocketFile(), failureExpected: false}}
	for _, tc := range testCases {
		client := NewClient(tc.sockFile, 1)
		response, err := client.RunCommand("show info", nil)
		if tc.failureExpected && err == nil {
			t.Errorf("TestClientRunCommand test case %s expected a failure but got none, response=%s", tc.name, string(response))
		}
		if !tc.failureExpected && err != nil {
...
func TestClientRunInfoCommandConverter(t *testing.T) {
	...
	testCases := []struct {
		...
		failureExpected	bool
	}{{name: "info parser", command: "show info", header: "name value", converter: nil, failureExpected: false}, {name: "info parser with comment header", command: "show info", header: "#name value", converter: nil, failureExpected: false}, {name: "info parser with bad header", command: "show info", header: "# name value extra1 extra2", converter: nil, failureExpected: true}, {name: "info parser with empty header", command: "show info", header: "", converter: nil, failureExpected: false}, {name: "bad command with header", command: "bad command", header: "field1 field2 field3", converter: nil, failureExpected: true}}
	server := haproxytesting.StartFakeServerForTest(t)
	defer server.Stop()
	for _, tc := range testCases {
		...
		if tc.failureExpected && err == nil {
			t.Errorf("TestClientRunInfoCommandConverter test case %s expected a failure but got none, response=%s", tc.name, string(response))
		}
		if !tc.failureExpected && err != nil {
...
func TestClientRunBackendCommandConverter(t *testing.T) {
	...
	testCases := []struct {
		...
		converter	ByteConverterFunc
		failureExpected	bool
	}{{name: "show backend command", command: "show backend", header: "name", converter: nil, failureExpected: false}}
	server := haproxytesting.StartFakeServerForTest(t)
	defer server.Stop()
	for _, tc := range testCases {
		client := NewClient(server.SocketFile(), 1)
		...
		response, err := client.RunCommand(tc.command, csvcon)
		if tc.failureExpected && err == nil {
			t.Errorf("TestClientRunBackendCommandConverter test case %s expected a failure but got none, response=%s", tc.name, string(response))
		}
		if !tc.failureExpected && err != nil {
...
func TestClientRunMapCommandConverter(t *testing.T) {
	...
	testCases := []struct {
		...
		converter	ByteConverterFunc
		failureExpected	bool
	}{{name: "show map command", command: "show map", header: "id (file) description", converter: fixupMapListOutput, failureExpected: false}, {name: "show map command with no converter", command: "show map", header: "id (file) description", converter: nil, failureExpected: true}}
	server := haproxytesting.StartFakeServerForTest(t)
	defer server.Stop()
	for _, tc := range testCases {
		client := NewClient(server.SocketFile(), 1)
		...
		response, err := client.RunCommand(tc.command, csvcon)
		if tc.failureExpected && err == nil {
			t.Errorf("TestClientRunMapCommandConverter test case %s expected a failure but got none, response=%s", tc.name, string(response))
		}
		if !tc.failureExpected && err != nil {
...
func TestClientRunServerCommandConverter(t *testing.T) {
	...
	testCases := []struct {
		...
		converter	ByteConverterFunc
		failureExpected	bool
	}{{name: "show servers state command", command: "show servers state be_edge_http:default:example-route", header: serversStateHeader, converter: stripVersionNumber, failureExpected: false}, {name: "show servers state command without a converter", command: "show servers state be_edge_http:default:example-route", header: serversStateHeader, converter: nil, failureExpected: true}}
	server := haproxytesting.StartFakeServerForTest(t)
	defer server.Stop()
	for _, tc := range testCases {
		client := NewClient(server.SocketFile(), 1)
		...
		response, err := client.RunCommand(tc.command, csvcon)
		if tc.failureExpected && err == nil {
			t.Errorf("TestClientRunServerCommandConverter test case %s expected a failure but got none, response=%s", tc.name, string(response))
		}
		if !tc.failureExpected && err != nil {
...
func TestClientExecute(t *testing.T) {
	...
	testCases := []struct {
		...
		command		string
		failureExpected	bool
	}{{name: "info command", command: "show info", failureExpected: false}, {name: "bad command", command: "bad command here", failureExpected: true}, {name: "show backend command", command: "show backend", failureExpected: false}, {name: "show map command", command: "show map", failureExpected: false}, {name: "show servers state command", command: "show servers state be_edge_http:default:example-route", failureExpected: false}, {name: "set server ipaddr command", command: "set server be_edge_http:default:example-route/_dynamic-pod-1 ipaddr 1.2.3.4", failureExpected: false}, {name: "set server ipaddr and port command", command: "set server be_edge_http:default:example-route/_dynamic-pod-1 ipaddr 1.2.3.4 port 8080", failureExpected: false}, {name: "set server weight command", command: "set server be_edge_http:default:example-route/_dynamic-pod-1 weight 256", failureExpected: false}, {name: "set server state command", command: "set server be_edge_http:default:example-route/_dynamic-pod-1 state maint", failureExpected: false}}
	server := haproxytesting.StartFakeServerForTest(t)
	defer server.Stop()
	for _, tc := range testCases {
		client := NewClient(server.SocketFile(), 1)
		...
		if tc.failureExpected && err == nil {
			t.Errorf("TestClientExecute test case %s expected a failure but got none, response=%s", tc.name, string(response))
		}
		if !tc.failureExpected && err != nil {
...
func TestClientCommit(t *testing.T) {
	...
	skipNames := map[string]bool{"be_sni": true, "be_no_sni": true, "openshift_default": true}
	server.Reset()
	for _, be := range backends {
		...
		if err := be.UpdateServerState(serverName, BackendServerStateMaint); err != nil {
			t.Errorf("TestClientCommit error setting state to maint for backend %s, server %s: %v", be.Name(), serverName, err)
		}
	}
	client.Commit()
	if len(server.Commands()) == 0 {
	...
	for _, be := range backends {
		...
		if err := be.UpdateServerState(serverName, BackendServerStateMaint); err == nil {
			t.Errorf("TestClientCommit error setting state to maint for backend %s, server %s, expected an error but got none", be.Name(), serverName)
		}
	}
	client.Commit()
	if len(server.Commands()) == 0 {
...
func TestClientFindMap(t *testing.T) {
	...
	testCases := []struct {
		...
		mapName		string
		failureExpected	bool
	}{{name: "non-existent map", mapName: "/a/b/c/d/e.map", failureExpected: true}, {name: "existing redirect map", mapName: "/var/lib/haproxy/conf/os_route_http_redirect.map", failureExpected: false}, {name: "existing sni passthru map", mapName: "/var/lib/haproxy/conf/os_sni_passthrough.map", failureExpected: false}, {name: "existing http be map", mapName: "/var/lib/haproxy/conf/os_http_be.map", failureExpected: false}, {name: "existing tcp be map", mapName: "/var/lib/haproxy/conf/os_tcp_be.map", failureExpected: false}, {name: "existing edge and reencrypt map", mapName: "/var/lib/haproxy/conf/os_edge_reencrypt_be.map", failureExpected: false}, {name: "bad backend name typo", mapName: "/var/lib/haproxy/conf/os_http_be.map.with.typo", failureExpected: true}}
	server := haproxytesting.StartFakeServerForTest(t)
	defer server.Stop()
	for _, tc := range testCases {
		client := NewClient(server.SocketFile(), 1)
