...
func TestBuildHAProxyMaps(t *testing.T) {
	...
	}{{name: "empty socket", sockFile: "", failureExpected: true}, {name: "valid socket", sockFile: server.SocketFile(), failureExpected: false}, {name: "non-existent socket", sockFile: "/non-existent/fake-haproxy.sock", failureExpected: true}}
...
func TestNewHAProxyMap(t *testing.T) {
	...
	testCases := []struct {
		name		string
		sockFile	string
	}{{name: "empty", sockFile: ""}, {name: "valid socket", sockFile: server.SocketFile()}, {name: "non-existent socket", sockFile: "/non-existent/fake-haproxy.sock"}}
	for _, tc := range testCases {
		client := NewClient(tc.sockFile, 0)
		if client == nil {
...
func TestHAProxyMapName(t *testing.T) {
	...
	testCases := []struct {
		...
		mapName		string
		failureExpected	bool
	}{{name: "empty socket", sockFile: "", mapName: "empty.map", failureExpected: true}, {name: "empty socket valid map", sockFile: "", mapName: "/var/lib/haproxy/conf/os_sni_passthrough.map", failureExpected: true}, {name: "valid socket", sockFile: server.SocketFile(), mapName: "/var/lib/haproxy/conf/os_http_be.map", failureExpected: false}, {name: "valid socket but invalid map", sockFile: server.SocketFile(), mapName: "missing.map", failureExpected: true}, {name: "valid socket but typo map", sockFile: server.SocketFile(), mapName: "/var/lib/haproxy/conf/os_http_be.map-1234", failureExpected: true}, {name: "non-existent socket", sockFile: "/non-existent/fake-haproxy.sock", mapName: "non-existent.map", failureExpected: true}, {name: "non-existent socket valid map", sockFile: "/non-existent/fake-haproxy.sock", mapName: "/var/lib/haproxy/conf/os_tcp_be.map", failureExpected: true}}
	for _, tc := range testCases {
		client := NewClient(tc.sockFile, 0)
		if client == nil {
			t.Errorf("TestHAProxyMapRefresh test case %s failed with no client.", tc.name)
