...
`
	...
	for _, tc := range testCases {
		...
		if tc.failureExpected && err == nil {
			t.Errorf("TestShowInfoCommandConverter test case %s expected a failure but got none, response=%s", tc.name, string(response))
		}
		if !tc.failureExpected && err != nil {
...
func TestShowServerStateOutputConverter(t *testing.T) {
	...
	testCases := []struct {
		...
		converter	ByteConverterFunc
		failureExpected	bool
	}{{name: "show servers state", commandOutput: haproxytesting.OnePodAndOneDynamicServerBackendTemplate, header: serversStateHeader, converter: stripVersionNumber, failureExpected: false}, {name: "show servers state without a converter", commandOutput: haproxytesting.OnePodAndOneDynamicServerBackendTemplate, header: serversStateHeader, converter: nil, failureExpected: true}, {name: "show servers state without removing version number", commandOutput: haproxytesting.OnePodAndOneDynamicServerBackendTemplate, header: serversStateHeader, converter: removeLeadingHashConverter, failureExpected: true}, {name: "show servers state removing first line with version number", commandOutput: haproxytesting.OnePodAndOneDynamicServerBackendTemplate, header: serversStateHeader, converter: removeFirstLineConverter, failureExpected: false}, {name: "show servers state with error converter", commandOutput: haproxytesting.OnePodAndOneDynamicServerBackendTemplate, header: serversStateHeader, converter: errorConverter, failureExpected: true}, {name: "show servers state with error output", commandOutput: "error: failed to find backend", header: serversStateHeader, converter: nil, failureExpected: true}}
	for _, tc := range testCases {
		entries := []*serverStateInfo{}
		c := NewCSVConverter(tc.header, &entries, tc.converter)
		response, err := c.Convert([]byte(tc.commandOutput))
		if tc.failureExpected && err == nil {
			t.Errorf("TestShowServerStateOutputConverter test case %s expected a failure but got none, response=%s", tc.name, string(response))
		}
		if !tc.failureExpected && err != nil {
